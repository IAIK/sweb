...
MinixFSInode::MinixFSInode(Superblock *super_block, uint16 i_mode, uint16 __attribute__((unused)) i_uid, uint32 i_size, uint32 __attribute__((unused)) i_modtime, uint8 __attribute__((unused)) i_gid, uint8 i_nlinks, uint16* i_zones, uint32 i_num) : Inode(super_block, 0)
{
  ...
  //std::cout << "new Inode: " << this << " size: " << i_size_ << " links: " << i_nlink_ << std::endl << " num zones: " << i_zones_->getNumZones() << " mode: " << i_mode << std::endl;
}
...
int32 MinixFSInode::readData(uint32 offset, uint32 size, char *buffer)
{
  ...
  {
    rbuffer->clear();
    ((MinixFSSuperblock*)i_superblock_)->readZone(i_zones_->getZone(zone), rbuffer->getBuffer());
...
{
  //std::cout << "MinixFSInode writeData> size: " << size << " i_size_: " <<  i_size_ << std::endl;
  ...
  if((size + offset) > i_size_)
  {
    ...
    //std::cout << "num_new_zones: " << num_new_zones << std::endl;
  ...
  if(offset > i_size_)
  {
    ...
    if (zone_size_offset)
    {
      ...
      ((MinixFSSuperblock *)i_superblock_)->writeZone( last_used_zone, fill_buffer->getBuffer());
    }
    ++last_used_zone;
...
int32 MinixFSInode::findDentry(uint32 i_num)
{
  ...
  {
    ((MinixFSSuperblock *)i_superblock_)->readZone(i_zones_->getZone(zone), dbuffer->getBuffer());
...
void MinixFSInode::writeDentry(uint32 dest_i_num, uint32 src_i_num, const char* name)
{
  ...
  ((MinixFSSuperblock *)i_superblock_)->readZone(zone, dbuffer->getBuffer());
  ...
  ((MinixFSSuperblock *)i_superblock_)->writeZone(zone, dbuffer->getBuffer());
  delete dbuffer;
...
int32 MinixFSInode::rmdir()
{
  ...
  for(uint32 i = 0; i < dentry->getNumChild(); i++)
  {
    ...
    {
      //std::cout << "removing directory not possible!" << std::endl;
      return -1;
    }
  }
...
Dentry* MinixFSInode::lookup(const char* name)
{
  ...
  if(i_type_ == I_DIR)
  {
    ...
    else
    {
      ...
      /*if(!dentry_update->getInode())
      {
        std::cout << "-------------something wrong here in dentry " << dentry_update << std::endl;
        return 0;
      }*/
