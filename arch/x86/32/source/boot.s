...
data_segment_ok:
   ...
   ; prepare core0 selector
   mov eax, core0_local_storage
   sub eax, 4096
   mov word[linear_core0_sel.low_word - BASE], ax
   shr eax, 16
   mov byte[linear_core0_sel.high_word_low - BASE], al
   shr eax, 8
   mov byte[linear_core0_sel.high_word_high - BASE], al
...
linear_core0_sel:
  ...
  dw 01h ; the limit, since the page-granular bit is turned on this is shifted 12 bits to the left
             ; in our case this means that the segment spawns the whole 32bit address space
.low_word:
  dw 00h   ; since it also starts at 0
.high_word_low:
  db 00h
  db 92h   ; 1 00 1 0010 == present, ring 0, data, expand-up, writable
  db 0CFh  ; 1 1 0 0 granularity:page (4 gig limit), 32-bit, not a 64 bit code segment
