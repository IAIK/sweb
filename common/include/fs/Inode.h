...
class Inode
{
 protected:
  ...
  // Elementar
  //--------------------------------------------------------------------------
  /// the sequece number of a inode, the Superblock search a patically inode
  /// with a secquece number.
  uint64 i_ino_;
  ...
  /// the device of the inode that this file-system is mounted on.
  // kdev_t i_dev_;
  // kdev_t i_rdev_;

  // Superblock *i_superblock_;

  /// the number of hard link in a inode.
  // ulink_t i_nlink_;
  ...
  /// the last used time of this inode.
  // time_t i_atime_;

  /// the last modify time of this inode.
  // time_t i_mtime_;

  /// the created time of this inode.
  // time_t i_ctime_;
  ...
  /// The i_version_ field is available for file-systems to use to record that
  /// a change has been made since some previous time. Typically the i_version
  /// is set to the current value of the event global variable which is then
  /// incremented. The file-system code will sometimes assign the current value
  /// of i_version_ to the f_version_ field of an associated file class.
  uint64 i_version_;

  /// This field records the number of pages, linked at i_pages_ which are
  /// currently cached for this inode. It is incremented by
  /// add_page_to_inode_queue and decremented by remove_page_from_inode_queue.
  uint64 i_nrpages_;

  /// the super block that the inode include in.
  // Superblock *i_sb_;

  /// The are three passible inode state bits: I_DIRTY, I_LOCK, I_FREEING.
  uint32 i_state_;

  /// The i_flags_ field correspond to the s_flags_ field in the super block.
  /// Many of the flags can be set system wide or per inode.
  uint32 i_flags_;

  // uint8 i_pipe_; // inode pipe
  // uint8 i_sock_; // inode socket
