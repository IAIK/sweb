...
MinixFSZone::MinixFSZone(MinixFSSuperblock *superblock, uint32 *zones)
{
  ...
  num_zones_ = 0;
  for (uint32 i = 0; i < NUM_ZONES(superblock_); i++)
  {
  ...
  if (zones[7])
  {
    indirect_zones_ = new uint32[NUM_ZONE_ADDRESSES(superblock_)];
    superblock_->readZone(zones[7], buffer);
    for (uint32 i = 0; i < NUM_ZONE_ADDRESSES(superblock_); i++)
    {
  ...
  if (zones[8])
  {
    double_indirect_zones_ = new uint32*[NUM_ZONE_ADDRESSES(superblock_)];
    double_indirect_linking_zone_ = new uint32[NUM_ZONE_ADDRESSES(superblock_)];
    superblock_->readZone(zones[8], buffer);
    char ind_buffer[ZONE_SIZE];
    for (uint32 ind_zone = 0; ind_zone < NUM_ZONE_ADDRESSES(superblock_); ind_zone++)
    {
      if (superblock_->s_magic_ == MINIX_V3)
      ...
      if (double_indirect_linking_zone_[ind_zone])
      {
        ...
        double_indirect_zones_[ind_zone] = new uint32[NUM_ZONE_ADDRESSES(superblock_)];

        for (uint32 d_ind_zone = 0; d_ind_zone < NUM_ZONE_ADDRESSES(superblock_); d_ind_zone++)
        {
          if (superblock_->s_magic_ == MINIX_V3)
  ...
  if (isDebugEnabled(M_ZONE))
  {
    ...
    for (uint32 i = 0; i < NUM_ZONES(superblock_); i++, print_num_zones++)
    {
      kprintfd("====zone: %x\t", direct_zones_[i]);
    }
    kprintfd("===indirect Zones:===\n");
    for (uint32 i = 0; i < NUM_ZONE_ADDRESSES(superblock_) && print_num_zones < num_zones_; i++, print_num_zones++)
    {
      kprintfd("===zone: %x\t", indirect_zones_[i]);
    }
    kprintfd("=dblindirect Zones:==\n");
    for (uint32 ind_zone = 0; ind_zone < NUM_ZONE_ADDRESSES(superblock_) && print_num_zones < num_zones_;
        ind_zone++, print_num_zones++)
    {
      for (uint32 d_ind_zone = 0; d_ind_zone < NUM_ZONE_ADDRESSES(superblock_) && print_num_zones < num_zones_;
...
uint32 MinixFSZone::getZone(uint32 index)
{
  ...
  index -= 7;
  if (index < NUM_ZONE_ADDRESSES(superblock_))
  {
    return indirect_zones_[index];
  }
  index -= NUM_ZONE_ADDRESSES(superblock_);
  return double_indirect_zones_[index / NUM_ZONE_ADDRESSES(superblock_)][index % NUM_ZONE_ADDRESSES(superblock_)];
}
...
void MinixFSZone::setZone(uint32 index, uint32 zone)
{
  ...
  if (!double_indirect_zones_[index / NUM_ZONE_ADDRESSES(superblock_)])
  {
    double_indirect_linking_zone_[index / NUM_ZONE_ADDRESSES(superblock_)] = superblock_->allocateZone();
    double_indirect_zones_[index / NUM_ZONE_ADDRESSES(superblock_)] = new uint32[NUM_ZONE_ADDRESSES(superblock_)];
    for (uint32 i = 0; i < NUM_ZONE_ADDRESSES(superblock_); i++)
      double_indirect_zones_[index / NUM_ZONE_ADDRESSES(superblock_)][i] = 0;
  }
  double_indirect_zones_[index / NUM_ZONE_ADDRESSES(superblock_)][index % NUM_ZONE_ADDRESSES(superblock_)] = zone;
...
void MinixFSZone::flush(uint32 i_num)
{
  ...
  for (uint32 index = 0; index < NUM_ZONES(superblock_); index++)
  {
    if (superblock_->s_magic_ == MINIX_V3)
  ...
  uint32 block = 2 + superblock_->s_num_inode_bm_blocks_ + superblock_->s_num_zone_bm_blocks_
      + ((i_num - 1) * INODE_SIZE(superblock_)) / BLOCK_SIZE;
  if (superblock_->s_magic_ == MINIX_V3)
    superblock_->writeBytes(block, ((i_num - 1) * INODE_SIZE(superblock_)) % BLOCK_SIZE + 24, 40, buffer);
  else
    superblock_->writeBytes(block, ((i_num - 1) * INODE_SIZE(superblock_)) % BLOCK_SIZE + 14, 18, buffer);
...
void MinixFSZone::freeZones()
{
  ...
  for (uint32 i = 0; i < NUM_ZONE_ADDRESSES(superblock_); i++)
    if (double_indirect_zones_[i])
      for (uint32 j = 0; j < NUM_ZONE_ADDRESSES(superblock_); j++)
