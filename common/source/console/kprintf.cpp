...
static char* getFBAddr(uint8 row, uint8 col, bool is_paging_set_up)
{
    return (char*)ArchCommon::getFBPtr(is_paging_set_up) + ((row * 80 + col) * 2);
}

static void clearFBrow(uint8 row, bool is_paging_set_up)
{
    memset(getFBAddr(row, 0, is_paging_set_up), 0, 80 * 2);
}

static void FBnewline(bool is_paging_set_up)
{
    uint8 next_row = getNextFBrow();
    clearFBrow(next_row, is_paging_set_up);
    setFBrow(next_row);
    setFBcol(0);
}

static void kputc(const char c, bool is_paging_set_up)
{
    if (c == '\n')
    {
        FBnewline(is_paging_set_up);
    }
    else
    {
        if (getFBcol() == 80)
        ...
        char* fb_pos = getFBAddr(row, col, is_paging_set_up);
        fb_pos[0] = c;
        fb_pos[1] = 0x02;
...
void kprintf_func(int ch, void* arg __attribute__((unused)))
{
    if (kprintf_initialized)
    {
        ...
        if ((system_state == RUNNING) && ArchInterrupts::testIFSet() &&
    ...
    {
        kputc(ch, false); // Used while booting
    }
}
 ...
 *
 * kprintfd being interrupted while holding the lock (e.g. due to a pagefault) would block all other
 * threads attempting to print anything (and cause deadlocks if the holding thread then waits for
 * one of the blocked threads) This is a tradeoff between readability of debug output and
 * performance/safety in rare edge cases
 *
 * Example scenario that would produce a deadlock:
 * kprintfd -> disable interrupts -> acquire lock (thread A) -> pagefault -> enable interrupts ->
 * debug() -> kprintfd -> deadlock
 *
 * Even a reentrant lock does not solve this problem:
 * kprintfd -> disable interrupts -> acquire lock (thread A) -> pagefault -> enable interrupts ->
 * read from disk -> thread A sleep until read finished (wait for ATA irq handler) -> ata irq
 * handler (in arbitrary thread context) blocks on kprintfd lock -> deadlock
 *
...
void kprintfd(const char* fmt, ...)
{
    ...
    {
        // WARNING: recursive call to kprintf while holding kprintfd lock! (e.g. due to pagefault in
        // kprintfd)

        writeLine2Bochs(
            "\n\n\033[1;31mWARNING: recursive call to kprintfd while holding kprintfd lock. No "
            "longer properly serializing debug output to prevent deadlock!\033[0;39m\n\n");
        ...
        // We can only do this since this won't break anything (aside from producing garbled output
        // on the debug console) Don't release any other locks this way!
