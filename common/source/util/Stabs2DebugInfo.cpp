...
void Stabs2DebugInfo::initialiseSymbolTable()
{
  ...
  {
    if (ArchMemory::get_PPN_Of_VPN_In_KernelMapping((size_t)current_stab / PAGE_SIZE, nullptr, nullptr) && (current_stab->n_type == N_FUN || current_stab->n_type == N_FNAME))
    {
...
void Stabs2DebugInfo::getCallNameAndLine(pointer address, const char*& mangled_name, ssize_t &line) const
{
  ...
  if (!this || function_symbols_.empty() ||
...
int Stabs2DebugInfo::readNumber(const char *& input)
{
  if (!input)
    return -1;
...
size_t Stabs2DebugInfo::putChar2Buffer(char*& buffer, char c, size_t& size)
{
  if (size <= 1)
    return -1;
  *buffer++ = c;
