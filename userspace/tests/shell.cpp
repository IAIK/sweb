...
void handle_command(char* buffer, int buffer_size)
{
    ...
    for (c = 0; c < buffer_size; c++) // Do check for null termination at end of for loop
                                      // to have one last run with \0
    {
        if (argsCount >= 10)
        {
            argsCount = 10;
            printf("Argument count is limited to 10 (no dynamic memory allocation) - all "
    ...
    else if (strcmp(command, "help") == 0)
    {
        printf("Command Help:\n"
               ...
               "exit [exit_code]      is really the only command that does something "
               "right now\n"
               "ls [path]             pseudo ls\n\n");
    }
    ...
    while ((cchar = getchar()) != EOF)
    {
        ...
        else if (cchar == '\t') // autocomplete
        {
            ...
            for (i = 0;
                 ...
                 chars_in_buffer < res.str_len && chars_in_buffer < BUFFER_SIZE - 1;
                 i++, chars_in_buffer++)
            {
